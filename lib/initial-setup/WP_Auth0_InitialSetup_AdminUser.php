...
class WP_Auth0_InitialSetup_AdminUser {
	...
	public function callback() {
		...
		$admin_user = WP_Auth0_Api_Client::signup_user( $data );
		
		if ( $admin_user === false ) {
			
			// The signup_user call failed, retry
			// TODO: better error messaging?
			wp_redirect( admin_url( "admin.php?page=wpa0-setup&step=3&profile=social&result=error" ) );
		} else {
			
			// The signup_user call succeeded, save admin information and go to the next step
